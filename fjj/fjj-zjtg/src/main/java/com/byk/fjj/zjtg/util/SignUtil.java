...
public class SignUtil {
    ...
    public static void isSignEquals(Map<String, Object> paramMap) {
        ...
        if(!sign.equals(md5Str)) {
            throw new ZjtgException(ResultCodeEnum.SIGN_ERROR);
        }
    }
    ...
    public static String sendRequest(Map<String, Object> paramMap, String url){
        ...
        try {
            ...
            log.info(String.format("--> 发送请求到商户平台（FAFU基金）：post data %1s", postdata));
            byte[] reqData = postdata.getBytes("utf-8");
            byte[] respdata = HttpUtil.doPost(url,reqData);
            result = new String(respdata);
            log.info(String.format("--> 商户平台（FAFU基金）应答结果：result data %1s", result));
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        return result;
