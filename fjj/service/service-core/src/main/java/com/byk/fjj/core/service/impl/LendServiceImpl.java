...
import com.byk.fjj.core.enums.ReturnMethodEnum;
...
public class LendServiceImpl extends ServiceImpl<LendMapper, Lend> implements LendService {
    ...
    //标的列表(分页)
    @Override
    public List<Lend> selectList(Long page, Long limit) {
        List<Lend> lendList = baseMapper.selectLendPageList(page,limit);
    ...
    public void makeLoan(Long lendId) {
        ...
        //发送同步远程调用
        JSONObject result = RequestHelper.sendRequest(paramMap, ZjtgConst.MAKE_LOAD_URL);
        ...
        for (Map.Entry<Integer, BigDecimal> entry : mapInterest.entrySet()) {
            ...
            if (lendItemReturnList.size() > 0 && currentPeriod.intValue() == lend.getPeriod().intValue()) {
                ...
                BigDecimal sumInterest = lendItemReturnList.stream()
                        ...
                        .reduce(BigDecimal.ZERO, BigDecimal::add);
                //最后一期应还本金 = 用当前投资人的总投资金额 - 除了最后一期前面期数计算出来的所有的应还本金
                BigDecimal lastInterest = lendItem.getExpectAmount().subtract(sumPrincipal);
